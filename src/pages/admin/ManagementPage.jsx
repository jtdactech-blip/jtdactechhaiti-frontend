...
const pageConfigs = {
  ...
  services: {
    title: "Services",
    subtitle: "Jesyon servis yo ak tarif yo sou menm estrikti tab ak apercu referans lan.",
    endpoint: "/services",
  ...
  invoices: {
    title: "Factures",
    subtitle: "Paj factures la bay menm layout card + tab kle ki nan imaj la.",
    endpoint: "/invoices",
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setInsights([
          "Done fallback yo chaje pou paj la rete bel pandan API a pa disponib.",
    ...
    <MainLayout>
      <div className="admin-page">
        ...
        <section className="table-shell">
          ...
          {loading ? (
            <TableSkeleton rows={6} columns={config.columns.length} />
          ) : rows.length ? (
            ...
            <EmptyState
              title={`Pa gen ${config.title.toLowerCase()} disponib`}
              description="Lè backend la voye done, tab la ap ranpli otomatikman."
            />
          )}
        </section>
      </div>
